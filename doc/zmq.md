...
The Nexia Core daemon can be configured to act as a trusted "border
router", implementing the nexia wire protocol and relay, making
...
The ZeroMQ feature in Nexia Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building nexiad:
...
For instance:
    ...
    $ nexiad -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/nexiad.tx.raw \
...
These options can also be provided in nexia.conf.
...
From the perspective of nexiad, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into nexiad directly. Furthermore, no information is
...
using. Nexiad appends an up-counting sequence number to each
